...
/***                        Include Files                            ***/
/***********************************************************************/
#include "atbm_phy_regs.h"
#include "atbm_Rf_Rx_Tx_Psm.h"
...
static void ROM_PHY_TX_Psm_addr_value(const atbm_uint16 *reg_addr_vec,const atbm_uint32 *reg_value_vec, atbm_uint32 addr_num)
{
	atbm_uint32 addr_idx;
...
{	
	...
	{
		if(GET_LMAC_REDUCE_POWER_CONSUMPTION(__atbm_le32_to_cpu(regval_sets->phy_params.compileMacro))){
...
static void PHY_RF_Abb_RcCal_Init(void)
{
	...
	ROM_HW_WRITE_REG_BIT(0xacc00A4,13,4,(0xf |(0xf<<5)));
